...
void pack(char* dir_path, char* archive_path) {
    ...
    archive = open(archive_path, O_CREAT | O_WRONLY, S_IWUSR | S_IRUSR | S_IRGRP | S_IROTH); // работа с архивом через дескриптор
    if (archive == -1) {
...
void _pack(int archive, char* src_path) {
    ...
    DIR *dir = opendir(src_path); //Функция opendir() открывает поток каталога и возвращает указатель на структуру типа DIR, которая содержит информацию о каталоге. 

    // Если передан путь к файлу, то просто пакуем инфу о файле и его содержимое
    if (dir == NULL) {
    ...
    // Запись информации о корневой директории в архив (о папка на которую дали путь)
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        char file_path[MAX_PATH_LENGTH] = {0};//инициализируем?
        snprintf(file_path, sizeof(file_path), "%s/%s", src_path, entry->d_name);
        _remove_extra_slash(file_path);

        // Если директория
        if (entry->d_type == DT_DIR) { //проверка на то что мы встретили директорию
            _pack(archive, file_path); //запускаем процесс упаковки для этой директории. Получается функция вызывает саму себя.
        }
        else {//значит встретили файл, упаковываем его.
            // Запись информации о файле
...
void _pack_info(int fd, path_type p_type, char* path) { //функция упаковки данных о файле или папке
    // Взять значение enum
    u8 type = (u8)p_type;//тип пути
    ...
    do {
        ...
        if (write(fd/*файл дескриптор*/, &type/*тип пути*/, u8sz) != u8sz) {
            printf("[Ошибка] Невозможно записать тип текущего расположения.\n");
            break;
        }

        // Запись в архив длины пути
        u64 file_path_len = strlen(path) + 1;//вычисляем длину пути
        if (write(fd, &file_path_len, u64sz) != u64sz) { //записываем в fd u64sz байт из &file_path_len(т.е записываем &file_path_len в fd)
            printf("[Ошибка] Не удалось записать длину пути текущего расположения.\n");
            break;
        }

        // Запись в архив пути
        if (write(fd, path, file_path_len) != file_path_len) {//записываем в fd file_path_len байт из path (т.е записываем path в fd)
            printf("[Ошибка] Не удалось записать путь текущего расположения.\n");
            break;
        }

        error = 0;//указываем, что ошибок нет
    } while (0);
